...
let evenNumbers = mixedNumbers.filter(isEvenNumbers) 
...
let totalPopulation = populations.reduce(function (accum, current) {
...
{"monster":"Sandmush","CP":25},{"monster":"Charaflier","CP":215},
...
//Code Here
let myStrongest = monstersInYourPocket.filter(monst => { 
  return monst.CP > 200;
})
...
let orderTotals = orders.map(ord => {
  return (ord.price * (1 + ord.tax))
})
...
let bobsTotal = purchases.reduce(function(tot, arr){ 
  if(arr.owner === "Bob"){
    tot += arr.price
  }return tot
},0);
